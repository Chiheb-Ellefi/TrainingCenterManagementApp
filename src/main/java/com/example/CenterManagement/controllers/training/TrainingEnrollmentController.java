...
@RestController
@RequestMapping("/api/v1/trainingsEnrollment")
...
public class TrainingEnrollmentController {
    ...
    @GetMapping("/{trainingId}/available-participants")
    public ResponseEntity<List<UserDto>> getAvailableParticipants(@PathVariable String trainingId) {
        return ResponseEntity.ok(trainingEnrollmentService.getAvailableParticipants(trainingId));
    }
    @Operation(
    ...
    @DeleteMapping("/{id}/unenroll/{email}")
    public ResponseEntity<String> unenrollTraining(@PathVariable String id, @PathVariable String email) {
        if (id == null || email == null) {
            throw new BadRequestException("The provided training id or pid is null");
        }

        trainingEnrollmentService.cancelTrainingEnrollment(email, id);
